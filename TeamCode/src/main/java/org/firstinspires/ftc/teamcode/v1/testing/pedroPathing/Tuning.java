package org.firstinspires.ftc.teamcode.v1.testing.pedroPathing;

import static org.firstinspires.ftc.teamcode.v1.testing.pedroPathing.Tuning.changes;
import static org.firstinspires.ftc.teamcode.v1.testing.pedroPathing.Tuning.drawCurrent;
import static org.firstinspires.ftc.teamcode.v1.testing.pedroPathing.Tuning.drawCurrentAndHistory;
import static org.firstinspires.ftc.teamcode.v1.testing.pedroPathing.Tuning.follower;
import static org.firstinspires.ftc.teamcode.v1.testing.pedroPathing.Tuning.stopRobot;
...
@TeleOp(name = "V1 Pedro Tuning", group = "v1")
public class Tuning extends SelectableOpMode {
    public static Follower follower;

    static PoseHistory poseHistory;

    static TuningTelemetry telemetryM;
...
class ForwardVelocityTuner extends OpMode {
    ...
    // On this robot, +X command drives backwards in tuner context; keep this tunable.
    public static double FORWARD_POWER_COMMAND = -1;
    ...
    public void start() {
        ...
        // Velocity tuners should run robot-centric so "forward" and "right" are deterministic.
    ...
    public void loop() {
        ...
        if (!end) {
            if (Math.abs(follower.getPose().getX() - startX) >= DISTANCE) {
                end = true;
                stopRobot();
            } else {
                follower.setTeleOpDrive(FORWARD_POWER_COMMAND,0,0,true);
...
class LateralVelocityTuner extends OpMode {
    ...
    public static double DISTANCE = 48;
    public static double RECORD_NUMBER = 10;
    public static double LATERAL_POWER_COMMAND = 1;
    ...
    public void loop() {
        ...
        if (!end) {
            if (Math.abs(follower.getPose().getY() - startY) >= DISTANCE) {
                end = true;
                stopRobot();
            } else {
                follower.setTeleOpDrive(0,LATERAL_POWER_COMMAND,0,true);
 ...
 */
class Drawing {
    private static final Object robotLook = new Object();
    private static final Object historyLook = new Object();

    /**
     * Drawing is intentionally disabled in the safe standalone tuner build.
     */
     ...
     */
    public static void drawRobot(Pose pose, Object style) {
    }
     ...
     */
    public static void drawPath(Path path, Object style) {
    }
     ...
     */
    public static void drawPath(PathChain pathChain, Object style) {
     ...
     */
    public static void drawPoseHistory(PoseHistory poseTracker, Object style) {
    }
...
class TuningTelemetry {
    ...
    private final String driveLimitsBanner;
    ...
    public void update() {
        if (telemetry != null) {
            telemetry.clearAll();
            telemetry.addLine(runtimeBanner);
            telemetry.addLine(DRAWING_BANNER);
            telemetry.addLine(driveMapBanner);
            telemetry.addLine(driveLimitsBanner);
